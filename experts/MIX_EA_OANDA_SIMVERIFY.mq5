...
//   XM版と同一変更。docs/gold_hour_gate_20260816.md
//   ⚠️除外取引はXM5枠で5件のみ。過剰適合の兆候が強くフォワードで要監視。効きが確認できない
//     場合はfalseに戻すこと。※OANDAサーバ時刻はXMと異なる可能性があり、時間帯の意味がずれる。
input bool   UseGoldHourGate   = true;   // PB GOLD 時間帯ゲート
// v2.5: PB GOLDの保有期間上限（GDD18_16）。H4の64バー＝約10.7日で強制決済する。
//   XM版と同一変更。docs/gold_dd2_final_20260818.md
//   長期停滞からの尾損失を限定する狙い。0で無効。
//   GDD35_07(損失後クールダウン)との併用は逆効果のため入れていない
...
input group "=== OANDA DD lab (SIMVERIFY only; 0=production-identical) ==="
input int    OafxLabMode              = 0;
input double OafxGJMinRange           = 0.30;
input double OafxGJMaxRange           = 1.00;
input double OafxGJBuffer             = 0.00;
input double OafxGJRR                 = 2.00;
input double OafxGJBoostMult          = 6.00;
input int    OafxGJDirection          = 0;     // 0=both, 1=BUY only
input int    OafxGJConfirmBars        = 1;
input double OafxGJBodyMinATR         = 0.00;
input double OafxGJExtensionMaxATR    = 99.0;
input int    OafxGJRetestLookback     = 1;
input double OafxGJRetestToleranceATR = 0.00;
input double OafxGJDriftMinRatio      = 0.00;
input double OafxGJDriftMaxRatio      = 99.0;
input int    OafxGJDriftPolicy        = 0;
input int    OafxGJRangeStart         = 0;
input int    OafxGJRangeEnd           = 9;
input int    OafxGJTradeEnd           = 12;
input int    OafxGJForceClose         = 22;
input double OafxGJSLRangeFraction    = 1.00;
input int    OafxOverlapMask          = 0;     // bit 1=SCA UJ, 2=RSI EU, 4=Carry, 8=PB UJ
input int    OafxOverlapLimit         = 1;
input int    OafxOverlapSide          = 0;     // 0=both, 1=SELL only
input int    OafxLossLookbackHours    = 24;
...
int OnInit()
{
   ...
   { SLEEVE x=z; x.enabled=En_SCA_GBPJPY; x.strat=ST_SCA; x.symbol=Sym_GBPJPY; x.tf=PERIOD_M15;
     ...
     if(OafxLabMode==1){ x.scaMinRange=OafxGJMinRange; x.scaMaxRange=OafxGJMaxRange; x.scaBuf=OafxGJBuffer; }
     if(OafxLabMode==2){ x.rr=OafxGJRR; x.scaBoostMult=OafxGJBoostMult; }
     if(OafxLabMode==6){ x.scaRangeStart=OafxGJRangeStart; x.scaRangeEnd=OafxGJRangeEnd;
                         x.scaTradeEnd=OafxGJTradeEnd; }
     if(OafxLabMode==7){ x.rr=OafxGJRR; x.scaForceClose=OafxGJForceClose; }
     AddSleeve(x); }
...
//============================ 指標ハンドル生成（検査付き・再生成可能） ============================
// 2026-08-13追加（XM版から移植）。従来はiMA/iATR等の戻り値を検査していなかったため、
// OnInit時点で対象銘柄のバーが1本も存在しないとhandle生成が失敗（error 4805）したまま
// 無音でスリーブが死んでいた。その状態ではCopyBufferで早期returnし**エントリーも決済も止まる**。
...
// 死んだハンドルを持つ枠を毎分1回だけ再生成する。建玉を保持したまま死んでいる場合は
// 決済も止まっている＝危険なので、その旨を明示して警告する。
datetime g_lastHandleCheck=0;
void EnsureHandles()
{
   datetime now=TimeCurrent();
   if(now-g_lastHandleCheck < 60) return;
   g_lastHandleCheck=now;
   for(int i=0;i<NS;i++)
   {
      if(!S[i].enabled) continue;
      if(S[i].strat==ST_PAIR) continue;
...
// 枠の判定はmagicで行う（銘柄名がXAUUSDで可変のため）。
...
// 時刻はブローカーのサーバ時刻。枠の判定はmagicで行う（銘柄名はXAUUSDで可変のため）。
...
bool OafxGJBreakQuality(const int i,const bool is_buy,const double level,const double atrd)
{
   if(S[i].magic!=20261001) return true;
   string sym=S[i].symbol; ENUM_TIMEFRAMES tf=S[i].tf;
   if(OafxLabMode==2 && OafxGJDirection==1 && !is_buy) return false;
   if(OafxLabMode==3)
   {
      int confirms=MathMax(1,OafxGJConfirmBars);
      for(int shift=1;shift<=confirms;shift++)
      {
         double closeN=iClose(sym,tf,shift);
         if((is_buy && closeN<=level) || (!is_buy && closeN>=level)) return false;
      }
      double body=MathAbs(iClose(sym,tf,1)-iOpen(sym,tf,1));
      double extension=is_buy ? iClose(sym,tf,1)-level : level-iClose(sym,tf,1);
      if(body<OafxGJBodyMinATR*atrd || extension>OafxGJExtensionMaxATR*atrd) return false;
   }
   if(OafxLabMode==4)
   {
      bool touched=false;
      int lookback=MathMax(1,OafxGJRetestLookback);
      double tolerance=OafxGJRetestToleranceATR*atrd;
      for(int shift=1;shift<=lookback;shift++)
      {
         if(is_buy && iLow(sym,tf,shift)<=level+tolerance){ touched=true; break; }
         if(!is_buy && iHigh(sym,tf,shift)>=level-tolerance){ touched=true; break; }
      }
      double extension=is_buy ? iClose(sym,tf,1)-level : level-iClose(sym,tf,1);
      if(!touched || extension>OafxGJExtensionMaxATR*atrd) return false;
   }
   if(OafxLabMode==5)
   {
      double width=S[i].scaRangeHigh-S[i].scaRangeLow;
      if(width<=0.0) return false;
      double ratio=MathAbs(S[i].scaDrift)/width;
      if(ratio<OafxGJDriftMinRatio || ratio>OafxGJDriftMaxRatio) return false;
      if(OafxGJDriftPolicy==1 && ((is_buy && S[i].scaDrift<=0) || (!is_buy && S[i].scaDrift>=0))) return false;
      if(OafxGJDriftPolicy==2 && ((is_buy && S[i].scaDrift>=0) || (!is_buy && S[i].scaDrift<=0))) return false;
      if(OafxGJDriftPolicy==3 && !is_buy && S[i].scaDrift>=0) return false;
   }
   return true;
}

bool OafxMaskHasMagic(const long magic,const int mask)
{
   if(magic==20261000 && (mask&1)!=0) return true;
   if(magic==20260605 && (mask&2)!=0) return true;
   if(magic==20260650 && (mask&4)!=0) return true;
   if(magic==20260622 && (mask&8)!=0) return true;
   return false;
}

bool OafxGJOverlapBlocked(const bool is_buy)
{
   if(OafxLabMode!=8 || (OafxOverlapSide==1 && is_buy)) return false;
   int count=0;
   for(int k=PositionsTotal()-1;k>=0;k--)
   {
      ulong ticket=PositionGetTicket(k); if(ticket==0) continue;
      if(OafxMaskHasMagic(PositionGetInteger(POSITION_MAGIC),OafxOverlapMask)) count++;
   }
   return count>=MathMax(1,OafxOverlapLimit);
}

bool OafxLossScopeMagic(const long magic)
{
   if(OafxLossScope==1) return magic==20261001;
   if(OafxLossScope==2) return magic==20261001 || magic==20261000;
   if(OafxLossScope==3) return magic>=20260000 && magic<20270000;
   if(OafxLossScope==4) return magic==20261000 || magic==20260605 || magic==20260650 || magic==20260622;
   return false;
}

bool OafxGJLossCapBlocked()
{
   if(OafxLabMode!=9 || OafxLossLookbackHours<=0 || OafxLossCapJPY<=0.0) return false;
   datetime now=TimeCurrent(), from=now-(datetime)(OafxLossLookbackHours*3600);
   ...
   for(int d=0;d<total;d++)
   {
      ...
      if(!OafxLossScopeMagic(HistoryDealGetInteger(deal,DEAL_MAGIC))) continue;
      double pnl=HistoryDealGetDouble(deal,DEAL_PROFIT)+HistoryDealGetDouble(deal,DEAL_SWAP)
                +HistoryDealGetDouble(deal,DEAL_COMMISSION);
      if(pnl<0.0) gross_loss-=pnl;
   }
...
void ProcSCA(int i)
{
   ...
   bool gj_loss_block=(S[i].magic==20261001 && OafxGJLossCapBlocked());

   // 上抜けブレイク → 買い
   double buy_level=S[i].scaRangeHigh+buffer;
   if(close1>buy_level && !hasB && !S[i].scaTradedL && !gj_loss_block
      && OafxGJBreakQuality(i,true,buy_level,atrd) && !OafxGJOverlapBlocked(true)){
      double ask=SymbolInfoDouble(sym,SYMBOL_ASK);
      double sl=S[i].scaRangeLow, dist=ask-sl;
      if(S[i].magic==20261001 && OafxLabMode==7)
   ...
   // 下抜けブレイク → 売り
   double sell_level=S[i].scaRangeLow-buffer;
   if(close1<sell_level && !hasS && !S[i].scaTradedS && !gj_loss_block
      && OafxGJBreakQuality(i,false,sell_level,atrd) && !OafxGJOverlapBlocked(false)){
      double bid=SymbolInfoDouble(sym,SYMBOL_BID);
      ...
      if(eqh!=INVALID_HANDLE){
         ...
         for(int e=0;e<n;e++){ ulong tk=HistoryDealGetTicket(e); if(tk==0) continue;
            ...
            FileWrite(eqh,(long)deal_time,DoubleToString(p,2),
                      ...
                      DoubleToString(uj,5),DoubleToString(p,2)); }
         FileClose(eqh);
      }
   }
