//+------------------------------------------------------------------+
//|  PortfolioEA_OANDA.mq5                                           |
//|  統合ポートフォリオEA（OANDA証券・国内版）                       |
//|  ベースは PortfolioEA.mq5（XM/汎用版）と同一ロジック。           |
//|  OANDA証券（国内）の仕様差を吸収するため以下を変更：             |
//|   (1) 全枠の銘柄名を input 化（サフィックス差・GOLD→XAUUSD）。    |
//|   (2) ETHUSD枠は OANDA に暗号資産CFDが無いため既定OFF（取扱不可）。|
//|   (3) XAUUSD は実機OANDA本番端末でFX枠と同一端末稼働を確認済み。  |
//|       → GOLD含む10枠を本EA1インスタンスで稼働可（ETHのみ除外）。 |
//|         ※口座にCFDアクセスが無い場合のみ En_PB_GOLD=false に。   |
//|  ※東京サーバーのMT5は両建て可能（ヘッジング相当）＝Magicごとの  |
//|    独立管理が成立する。                                          |
...
int OnInit()
{
   ...
   Print("PortfolioEA_OANDA v1.0 起動 | 有効枠数=", CountEnabled(), "/", NS,
