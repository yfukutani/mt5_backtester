...
#property version   "1.60"   // v1.6: RangeMode(実体/終値/分位/M5) / BreakOnWick / TiltGate / DecayBuffer / OppTouch（第2バックログ検証用・既定全OFF）
#property strict
...
input double Buf_DecayPerHour   = 0.0;   // D2-5: 遅いブレイクほどバッファ加算（ATRd倍率/時）
input bool   RequireOppTouch    = false; // D2-7: 反対端タッチ後のブレイクのみ（振り落とし確認）
...
bool     g_touchedLow  = false; // 当日レンジ下端タッチ済み
...
bool ComputeRange(datetime day_start)
{
    ...
    bool haveL = false;
    double hArr[400], lArr[400];
    int cnt = 0;

    // C2-8: M5精密レンジは下位足で窓を走査
    ENUM_TIMEFRAMES tf = (RangeMode == 4 ? PERIOD_M5 : PERIOD_CURRENT);
    int maxScan = (RangeMode == 4 ? 400 : 200);
    int bars = Bars(_Symbol, tf);
    for(int sft = 1; sft < maxScan; sft++)
    {
        if(sft >= bars) break;
        datetime bt = iTime(_Symbol, tf, sft);
        if(bt < t_from) break;           // 窓より過去に出たら終了
        if(bt >= t_to) continue;         // 窓より未来（レンジ後）はスキップ
        double o = iOpen(_Symbol, tf, sft);
        double c = iClose(_Symbol, tf, sft);
        double h, l;
        if(RangeMode == 1)      { h = MathMax(o, c); l = MathMin(o, c); }   // 実体
        else if(RangeMode == 2) { h = c; l = c; }                            // 終値のみ
        else                    { h = iHigh(_Symbol, tf, sft); l = iLow(_Symbol, tf, sft); }
        if(cnt < 400) { hArr[cnt] = h; lArr[cnt] = l; cnt++; }
        if(h > hi) hi = h;
        if(l < lo) lo = l;
        if(!haveL) { closeL = c; haveL = true; }   // 窓内最新バー
        openF = o;                                 // 窓内最古バー
    }
    if(hi <= -DBL_MAX || lo >= DBL_MAX || cnt == 0) return false;

    // C2-2: 分位レンジ（外れ値バー1本の影響を除く: highの90%分位 / lowの10%分位）
    if(RangeMode == 3 && cnt >= 5)
    {
        double hh[], ll[];
        ArrayResize(hh, cnt);
        ArrayResize(ll, cnt);
        ArrayCopy(hh, hArr, 0, 0, cnt);
        ArrayCopy(ll, lArr, 0, 0, cnt);
        ArraySort(hh);
        ArraySort(ll);
        hi = hh[(int)MathFloor(0.9 * (cnt - 1))];
        lo = ll[(int)MathCeil(0.1 * (cnt - 1))];
...
void OnTick()
{
    ...
    // D2-5: 遅いブレイクほどバッファを厚く
    double buffer  = (Break_Buffer_ATRd +
                      Buf_DecayPerHour * MathMax(0, dt.hour - RangeEndHour)) * atrd;
    double close1  = iClose(_Symbol, PERIOD_CURRENT, 1);
    double high1   = iHigh(_Symbol, PERIOD_CURRENT, 1);
    double low1    = iLow(_Symbol, PERIOD_CURRENT, 1);
    // D2-7: 反対端タッチ状態の更新
    if(high1 >= g_rangeHigh) g_touchedHigh = true;
    if(low1  <= g_rangeLow)  g_touchedLow  = true;
    // D2-1: ブレイク判定値（終値 or 確定バー高安）
    double refUp = (BreakOnWick ? high1 : close1);
    double refDn = (BreakOnWick ? low1  : close1);
    ...
    if(d1_ok_buy && refUp > g_rangeHigh + buffer && !has_buy && !(OneShotPerDir && g_tradedLong)
       && (!UseTiltGate || g_tilt > 0)
       && (!RequireOppTouch || g_touchedLow)
    ...
    if(d1_ok_sell && refDn < g_rangeLow - buffer && !has_sell && !(OneShotPerDir && g_tradedShort)
       && (!UseTiltGate || g_tilt < 0)
       && (!RequireOppTouch || g_touchedHigh)
