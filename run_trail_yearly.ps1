# Trailing stop yearly backtest 2016-2026
# H4: Trail_Start=30pips, Trail_Stop=20pips
...
$charts = @(
    [ordered]@{ label="USDJPY_H4"; symbol="USDJPY"; period="H4"; sl=50; tp=110; useDP="true";  dpBars=100; magic=20260610; csvName="trail_h4_result.csv"; trailStart=30; trailStop=20 },
    [ordered]@{ label="USDJPY_H1"; symbol="USDJPY"; period="H1"; sl=45; tp=105; useDP="false"; dpBars=60;  magic=20260604; csvName="trail_h1_result.csv"; trailStart=25; trailStop=15 },
    [ordered]@{ label="EURUSD_H1"; symbol="EURUSD"; period="H1"; sl=45; tp=105; useDP="false"; dpBars=60;  magic=20260605; csvName="trail_eu_result.csv"; trailStart=25; trailStop=15 }
)
...
Write-Host ""
Write-Host "===== Trailing Stop Yearly Results (H4:30/20pips, H1/EU:25/15pips) =====" -ForegroundColor Yellow
Write-Host ("{0,-6} {1,12} {2,12} {3,12} {4,10} {5,10} {6,8}" -f "Year","USDJPY_H4","USDJPY_H1","EURUSD_H1","Trail-Tot","Base-Tot","Diff")
Write-Host ("-" * 74)
...
foreach ($year in $years) {
    ...
    $grandTrail += $total
    $grandBase  += $base
    $lineColor = if ($diff -gt 0) { "Green" } elseif ($diff -lt -1000) { "Red" } else { "White" }
    Write-Host ("{0,-6} {1,12} {2,12} {3,12} {4,10} {5,10} {6,8}" -f $year, (Format-Profit $h4), (Format-Profit $h1), (Format-Profit $eu), (Format-Profit $total), (Format-Profit $base), (Format-Profit $diff)) -ForegroundColor $lineColor
}

$grandDiff = $grandTrail - $grandBase
Write-Host ("-" * 74)
Write-Host ("Grand  {0,45} {1,10} {2,8}" -f (Format-Profit $grandTrail), (Format-Profit $grandBase), (Format-Profit $grandDiff))
$diffColor = if ($grandDiff -ge 0) { "Green" } else { "Red" }
Write-Host ("Overall diff vs base: {0}" -f (Format-Profit $grandDiff)) -ForegroundColor $diffColor
Write-Host ""
