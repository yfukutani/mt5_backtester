...
【事前の予想（後付けを避けるため先に書く）】
- **X001（SCA GBPJPY の09時除外）**: 切り直しの見積もりは 枠純益
  **OOS +13,093 / IS +7,646**。**だが EA を読むと、そこまで出ないほうに賭けるべきである。**

  `ProcSCA()` の「1日1発」フラグ `scaTradedL` / `scaTradedS` は、
  **発注が成功したときにしか立たない**。したがって `ScaFilEntryOK` が09時を止めても
  その日の枠は終わらず、ブレイク条件（`close1 > scaRangeHigh + buffer`・GBPJPY は
  `buffer=0`）が成立し続けるかぎり **10時・11時の足で改めて建てる**
  （発注可能時間は 9・10・11時）。
  SL は `scaRangeLow` 固定なので**遅く入るほど SL 距離が伸び**、TP も遠くなる。
  ロットは `useRisk=false` の固定 0.01 で変わらず、`RevBoost` は `scaDrift`
  （レンジ窓内のドリフト）で決まるので時間帯に影響されない。
  利益の大半は22時の強制決済から出ている（TP到達は 3〜7%）ので、
  **「より高く買って同じ時刻に決済する」＝素直に不利**である。

  **したがって X001 の改善は切り直しより小さく、符号が逆になることもありうる。**
  **取引ログの切り直しは行を消すだけだが、EA は消さずに後ろへずらす。**
