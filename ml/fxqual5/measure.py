...
- **X002（SCA 2枠に同じ時間帯フィルタ）**: 切り直しでは SCA USDJPY の発注時刻は
  9・10・11時の3つで、IS ではすべて黒字なので **B2 は何も切らない（±0）**と報告されている。
  **X002 と X001 の差が SCA USDJPY 枠で 0 でなければ、切り直しは実測と一致しない**
  ことになる。**これがこのラウンドの本当の検算である。**
...
PROPOSALS = [
    ...
    ("X002", "X001",
     "SCA 2枠に同じ時間帯フィルタ。切り直しでは USDJPY 枠は ±0 のはず（検算）",
