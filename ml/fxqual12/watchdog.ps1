...
# したがって「終わったか」は exit code ではなく `measure.log` の `FXQUAL12_END` で判定する。
#
# ⚠️ 照合は `*fxqual12\chain.ps1*` と名指しする。`*fxqual12*` だと
#    このウォッチドッグ自身のパスに必ずヒットして永久に「生きている」と誤判定する。
# ⚠️ このファイルは UTF-8 BOM付きで保存すること。
$ErrorActionPreference = 'Continue'
$repo  = 'C:\Users\f\source\repos\mt5_backtester'
$mlog  = Join-Path $repo 'ml\fxqual12\measure.log'
$wlog  = Join-Path $repo 'ml\fxqual12\watchdog.log'
$chain = Join-Path $repo 'ml\fxqual12\chain.ps1'
...
Say 'WATCHDOG_START fxqual12 を見張る（5分おき・最大12時間）'
$deadline = (Get-Date).AddHours(12)
$restarts = 0
while ((Get-Date) -lt $deadline) {
  Start-Sleep -Seconds 300

  if ((Test-Path $mlog) -and (Select-String -Path $mlog -Pattern 'FXQUAL12_END|FXQUAL12_ABORT' -Quiet)) {
  ...
  $ch = @(Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.CommandLine -like '*fxqual12\chain.ps1*' })
  $mp = @(Get-CimInstance Win32_Process -Filter "Name='python.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.CommandLine -like '*fxqual12*measure.py*' })
  if ($ch.Count -gt 0 -or $mp.Count -gt 0) { continue }

  # 前段の fxqualexec がまだなら、動いていないのは正常
  $prev = Join-Path $repo 'ml\fxqualexec\measure.log'
  if ((Test-Path $prev) -and -not (Select-String -Path $prev -Pattern 'FXQUALEXEC_END|FXQUALEXEC_ABORT' -Quiet)) { continue }
