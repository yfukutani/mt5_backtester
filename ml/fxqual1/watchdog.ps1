...
#
# 【なぜ要るか】
# 2026-09-18 01:06 JST に fxvmax1 / pipeline_xm / fxqual1 のチェーンが**全部**落ちた。
...
$mlog = Join-Path $repo 'ml\fxqual1\measure.log'
$wlog = Join-Path $repo 'ml\fxqual1\watchdog.log'
...
Say 'WATCHDOG_START fxqual1 を見張る（5分おき・最大14時間）'
$deadline = (Get-Date).AddHours(14)
$restarts = 0
while ((Get-Date) -lt $deadline) {
  Start-Sleep -Seconds 300

  if ((Test-Path $mlog) -and (Select-String -Path $mlog -Pattern 'FXQUAL1_END|FXQUAL1_ABORT' -Quiet)) {
  ...
  $mp = @(Get-CimInstance Win32_Process -Filter "Name='python.exe'" -ErrorAction SilentlyContinue |
           Where-Object { $_.CommandLine -like '*fxqual1*measure.py*' })
  ...
  Start-Process -FilePath $py -ArgumentList 'ml\fxqual1\measure.py' -WorkingDirectory $repo `
    -WindowStyle Hidden `
    -RedirectStandardOutput (Join-Path $repo 'ml\fxqual1\stdout.log') `
    -RedirectStandardError  (Join-Path $repo 'ml\fxqual1\stderr.log')
}
