# fxqualfloor を fxqualamp の後ろに繋ぐ（第17報の追加ラウンド）。
#
# **EA は触らない**（使うのは既存 input だけ）。デプロイもコンパイルもしない。
# 待つのは「fxqualamp が終わってロックが空き、テスターが居ないこと」。
#
# ⚠️ 並行セッションがさらに別のラウンドを積む可能性があるので、
#    ロックが空くまで粘る（最大12時間）。measure.py の acquire_lock() に負けると
#    **何もせずに正常終了する**ので、ウォッチドッグ側で FXQUALFLOOR_END を見て起こし直す。
#
...
$log  = Join-Path $repo 'ml\fxqualfloor\chain.log'
...
Say 'CHAIN_START(fxqualfloor) fxqualamp の完了とロックの解放を待つ（最大12時間）'
$qprev = Join-Path $repo 'ml\fxqualamp\measure.log'
$deadline = (Get-Date).AddHours(12)
while ((Get-Date) -lt $deadline) {
  if ((Test-Path $qprev) -and (Select-String -Path $qprev -Pattern 'FXQUALAMP_END|FXQUALAMP_ABORT' -Quiet)) {
    if (-not (Busy)) { break }
  }
...
Say 'FXQUALFLOOR を開始する（レンジ幅下限の内点を挟む・6案 × 2窓 = 12run・EAは触らない）'
Set-Location $repo
& $py 'ml\fxqualfloor\measure.py' 2>&1 | Add-Content -Path $log -Encoding utf8
