...
- **`G006`（枠を丸ごと外す）が下限掃引の上限値になる**と予想する。
  つまり **どの掃引点も `G006` を超えない**。超えたら「狭いレンジを捨てる」だけでなく
  「残した取引の質も上がっている」ことになり、機構の理解が足りていない。
  **この一点だけは、外れたときの情報量が大きい。**
...
ORDER = ["G000", "G006", "G004", "G002", "G007", "G001", "G003", "G005"]
# 対照 → 上限値（枠外し）→ 帯の中心 → 帯の左 → F006 隣接点 → 端、の順。
