...
# したがって「終わったか」は exit code ではなく `measure.log` の `FXQUAL17_END` で判定する。
#
# ⚠️ 照合は `*fxqual17\chain.ps1*` と名指しする。`*fxqual17*` だと
...
$wlog  = Join-Path $repo 'ml\fxqual17\watchdog.log'
$chain = Join-Path $repo 'ml\fxqual17\chain.ps1'
...
Say 'WATCHDOG_START fxqual17 を見張る（5分おき・最大12時間）'
$deadline = (Get-Date).AddHours(12)
$restarts = 0
while ((Get-Date) -lt $deadline) {
  Start-Sleep -Seconds 300

  if ((Test-Path $mlog) -and (Select-String -Path $mlog -Pattern 'FXQUAL17_END|FXQUAL17_ABORT' -Quiet)) {
  ...
  $ch = @(Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.CommandLine -like '*fxqual17\chain.ps1*' })
  $mp = @(Get-CimInstance Win32_Process -Filter "Name='python.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.CommandLine -like '*fxqual17*measure.py*' })
  if ($ch.Count -gt 0 -or $mp.Count -gt 0) { continue }

  # 前段の fxqual16 がまだなら、動いていないのは正常

  $prev = Join-Path $repo 'ml\fxqual16\measure.log'
  if ((Test-Path $prev) -and -not (Select-String -Path $prev -Pattern 'FXQUAL16_END|FXQUAL16_ABORT' -Quiet)) { continue }
