...
**並行セッションが受動的な計装を EA に入れた**（`cc86514` → `02313d9`・`TrackMarginLevel()`）ので、
**最小維持率を初めて実測する。**

`02313d9` で **`ACCOUNT_MARGIN_LEVEL` が 0 を返す場合に `equity / 使用証拠金 × 100` を
自前で計算する経路**と、切り分け用のカウンタ（`margin_level_src` 行の
`used_ticks` / `from_api` / `computed` / `samples`）が足された。
**`used_ticks != samples` なら測れなかったティックがある。`computed > 0` なら
`ACCOUNT_MARGIN_LEVEL` はこのテスターでは使えていない。**
追加はどちらも読み取りとカウンタだけで、ロット・発注・決済には触れていない（差分を確認済み）。
