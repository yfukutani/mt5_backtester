"""OANDA FX 9枠のRefCap・倍率・重み24案をFULL/OOSで測る直列・再開可能な掃引。
複利対象はPB USDJPY・PB GBPJPY・Carryのみ。破綻の暫定代理指標も保存する。
端末・EA差し替え検知・スリープ抑止・枠別集計はfxmult1/fxweight1を継承する。
"""
...
def run(prop, window):
    ...
    run_id = f"fc_{window.lower()}_{prop['proposal_id']}_{stamp}_{uuid.uuid4().hex[:4]}"
    p = dict(BASE)
...
def mark_blown(row):
    ...
    if row["proposal_id"] == "C001" and row["status"] == "OK" and trades is not None and trades > 0:
...
def main():
    ...
    try:
        log(f"FXCOMP_START jobs={len(jobs)} done={len(done)}")
        for prop, window in jobs:
            # C001未取得で後続を測ると、破綻を比較判定できないため中断する。
            if prop["proposal_id"] != "C001" and window not in _baseline_trades:
                raise RuntimeError(f"{window}のC001基準取引数がありません")
            row = run(prop, window)
            mark_blown(row)
            append_result(row)
            if prop["proposal_id"] == "C001" and window not in _baseline_trades:
                raise RuntimeError(f"{window}のC001取得失敗。再開時に再試行してください")
    finally:
        ctypes.windll.kernel32.SetThreadExecutionState(0x80000000)
    log(f"FXCOMP_END -> {OUT}")
