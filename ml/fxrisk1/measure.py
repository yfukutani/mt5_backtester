"""OANDA FX 9枠のうち固定ロット5枠のrisk%化44案をFULL/OOSで比較する。
枠ごとの寄与と複利の影響を分離し、破綻の暫定代理指標も保存する。
再開・EA差し替え検知・スリープ抑止・端末回収はfxcomp1を継承する。
"""
...
def run(prop, window):
    ...
    run_id = f"fr_{window.lower()}_{prop['proposal_id']}_{stamp}_{uuid.uuid4().hex[:4]}"
    p = dict(BASE)
...
def mark_blown(row):
    ...
    if row["proposal_id"] == "R001" and row["status"] == "OK" and trades is not None and trades > 0:
...
def main():
    ...
    try:
        log(f"FXRISK_START jobs={len(jobs)} done={len(done)}")
        for prop, window in jobs:
            # R001未取得で後続を測ると、破綻を比較判定できないため中断する。
            if prop["proposal_id"] != "R001" and window not in _baseline_trades:
                raise RuntimeError(f"{window}のR001基準取引数がありません")
            row = run(prop, window)
            mark_blown(row)
            append_result(row)
            if prop["proposal_id"] == "R001" and window not in _baseline_trades:
                raise RuntimeError(f"{window}のR001取得失敗。再開時に再試行してください")
    finally:
        ctypes.windll.kernel32.SetThreadExecutionState(0x80000000)
    log(f"FXRISK_END -> {OUT}")
