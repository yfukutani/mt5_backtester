"""🔴 **この判別は使わないこと。`boost_split2.py` / `boost_check.py` を使う。**

**残してあるのは、なぜ不良だったかの記録のためである。消すと同じ穴を踏む。**

【何が悪かったか】
equity の代理に **確定損益の累積（残高）** を使い、リスク率 1.5% を閾値にした。
そして **「分布がきれいに二峰だから誤判定はほぼ無い」と書いた。分位の読み違いである。**
IS の `sca_gj` は p75 0.962 / p90 3.220 で、**15%（約102取引）が谷に落ちていた。**
**Carry が数か月〜数年ポジションを持つので、確定残高は equity の代理にならない**
（リスク率の max が設計値 3.0% の2倍の 6.106% になっていた）。

並行セッションの初稿（`w = R ÷ (0.5% × 確定残高) > 2.0`）も同じ穴で、
谷に OOS 18.0% / IS 30.0% が居た。**2セッションが独立に同じ間違いをした。**

【教訓】
**「二峰だから安全」と言う前に、谷にいくつ落ちているかを数える。**
**長期保有の枠があるブックでは、equity を確定損益で代理してはいけない。**

【正しい判別（規約・2026-09-19 に両セッションで固定）】
**「そのリスク額 ÷ 近傍±20取引のリスク額の中央値」が 3.0 以上なら boost。**
窓幅は前後20で飽和し（10→150/151・20→159/161・40→159/161）、
閾値 2.5〜3.5 でも自己を含めても1件も動かない。

---

（以下は当時の記述。**上記のとおり判別は不良である。**）
