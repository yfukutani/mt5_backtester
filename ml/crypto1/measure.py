"""暗号枠の改善を固定ロット増量と比較するため、XM本番構成とS2046を基準にする。

USDJPYチャートは、XMの古いXAUJPY履歴不足によるOOS失敗を避けるため維持する。
判定はadjudicate.pyでdealの円建て落ち込みを用い、純益倍率とDD倍率を直接比較する。
"""
...
MAGICS = {20260640: "pb", 20261002: "sca1", 20261003: "sca2", 20260710: "eth", 20260720: "fund", 20260724: "bfx"}
...
FIELDS = ["proposal_id", "family", "window", "status", "net", "pf", "dd_pct",
          "monthly_pct", "trades",
          "pb_net", "pb_n", "sca1_net", "sca1_n", "sca2_net", "sca2_n", "eth_net", "eth_n", "fund_net", "fund_n", "bfx_net", "bfx_n",
...
def run_one(name, exe, prop, window):
    ...
    run_id = f"cr_{window.lower()}_{prop['proposal_id']}_{stamp}_{uuid.uuid4().hex[:4]}"
    ...
    for key in ("pb", "sca1", "sca2", "eth", "fund", "bfx"):
        if key in st:
    ...
    log(f"RUN_END {prop['proposal_id']} {window} net={row.get('net')} "
        ...
        f"sca2_net={row.get('sca2_net')} fund_n={row.get('fund_n')} "
        f"fund_net={row.get('fund_net')} bfx_n={row.get('bfx_n')} "
        f"bfx_net={row.get('bfx_net')} {row['elapsed']}s")
    return row
