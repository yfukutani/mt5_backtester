...
> [!danger] **2026-09-17 夕の変更: cap を 90 → 50 にした。**
> 第14報（`docs/oanda_stopout_level_20260917.md`）で、**OANDA は証拠金維持率 100% で切る**
> ことが分かった（XM は 20%）。`MarginCapPct` は**発注時**の上限なので、
> cap90 で建てた直後の維持率は約 **111%** ——OANDA では 11% 逆行しただけでロスカットになる。
> 実際、同一構成（E15）を OANDA で回すと **4か月でテストごと打ち切られた。**
>
> **cap90 の XM 実測は「XM でしか成立しない数」である。** 窓の頑健性を測るなら、
> **本番で建てられる範囲**で測らなければ意味がない。cap50 なら建てた直後の維持率は
> **200%** で、OANDA のロスカット水準に対して余裕がある。
> **この変更で本ラウンドの数字は下がる。** 下がった数字のほうが本物である。
...
PROPOSALS = [
    ("Q001", "E04", "Carry＋PB UJ を外す・倍率3・cap50（維持率200%）",
     oa(3, 50, (CARRY, PBUJ))),
    ("Q003", "E15", "Carry＋PB UJ＋RSI EU を外す・倍率3・cap50",
     oa(3, 50, (CARRY, PBUJ, RSIEU))),
    ("Q002", "E09", "Carry＋PB UJ を外す・倍率4・cap50", oa(4, 50, (CARRY, PBUJ))),
    ("Q000", "Y050", "対照: X005（9枠すべて）・倍率3・cap50", oa(3, 50)),
    # cap90 を1本だけ残す。「cap を下げたぶんいくら落ちたか」を同じ窓で見るため。
    # **これは XM でしか成立しない構成**である（OANDA では4か月で飛ぶ）。
    ("Q005", "E04", "Carry＋PB UJ を外す・倍率3・cap90（XM専用・落差の比較用）",
     oa(3, 90, (CARRY, PBUJ))),
...
ORDER = ["Q001", "Q003", "Q005", "Q004", "Q002", "Q000"]
