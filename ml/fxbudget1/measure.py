...
PROPOSALS = [
    ...
    # --- 土俵は cap50 にする（第14報・2026-09-17 夕の変更）---------------------
    #     OANDA は証拠金維持率 **100%** で切る（XM は 20%）。
    #     `MarginCapPct` は発注時の上限なので、cap90 の直後の維持率は約111%で、
    #     OANDA では 11% 逆行しただけでロスカットになる（実測: 同一構成が4か月で打ち切り）。
    #     cap50 なら建てた直後の維持率は 200% で、本番でも建てられる。
    #     **B000 だけは cap90 のまま残す** —— fxvmax1 の O003 を1円まで再現することで
    #     「Bud_* を足した EA が既定挙動を壊していない」ことを確認するための対照だから。
    ("B050", "S050", "対照: 予算なし・cap50（以降の比較の土台）", bud(cap=50)),

    # --- 本題: RSI EURUSD の予算を刻む（「全停止」と「効かない」の間）-----------
    ("B001", "B050", "RSI EU の1注文を equity の 10% までに制限",
     bud(cap=50, Bud_RSI_EURUSD=10.0)),
    ("B002", "B050", "RSI EU を 20% まで", bud(cap=50, Bud_RSI_EURUSD=20.0)),
    ("B003", "B050", "RSI EU を 30% まで", bud(cap=50, Bud_RSI_EURUSD=30.0)),
    ("B004", "B050", "RSI EU を 40% まで", bud(cap=50, Bud_RSI_EURUSD=40.0)),

    # --- PairTrade は「通過量」の 48.9% を占める最大の消費者（第10報 §3）--------
    #     Pair は1回の希望が小さく通過率 88.1%＝**cap の恩恵を最も受けている枠**。
    #     RSI EU から証拠金を奪っているのは Pair ではないか、を測る。
    ("B005", "B050", "Pair の1注文を equity の 10% までに制限",
     bud(cap=50, Bud_PAIR=10.0)),
    ("B006", "B050", "Pair を 20% まで", bud(cap=50, Bud_PAIR=20.0)),

    # --- risk%枠をまとめて絞る（一律版・#9）------------------------------------
    ("B007", "B050", "RSI 3枠を一律 20% まで",
     bud(cap=50, Bud_RSI_EURUSD=20.0, Bud_RSI_GBPUSD=20.0, Bud_RSI_USDJPY=20.0)),
    ("B008", "B050", "全9枠を一律 20% まで",
     bud(cap=50, **{k: 20.0 for k in BUD_KEYS})),

    # --- 勝ち残った1本と Carry 停止（fxeff1 E01）の組み合わせ -------------------
    #     Carry は W2/W3/通期のすべてで負けている（第10報 §4b）。
    ("B009", "B050", "RSI EU 20% ＋ Carry の予算を 2%（ほぼ止めるが分散は残す）",
     bud(cap=50, Bud_RSI_EURUSD=20.0, Bud_CARRY=2.0)),
]

# 対照 → RSI EU の刻み → Pair → 一律 の順。途中で止まっても判断に効く順に並べる。
ORDER = ["B000", "B050", "B002", "B003", "B001", "B004", "B005", "B009",
         "B007", "B006", "B008"]
