...
def main():
    ...
    for r in read_rows(ROOT / 'proposals.csv'):
        if r['family'] == 'T1_window':
            p = json.loads(r['parameter_json'])
            start = int(p['Sca3RangeStart'])
            grid[start, int(p['Sca3RangeEnd']) - start] = r['proposal_id']
    ...
    rows = {win: {r['proposal_id']: r for r in read_rows(ROOT / 'results.csv')
                  ...
                  and r['family'] == 'T1_window' and r.get('sca3_net') not in (None, '')}
            for win in WINDOWS}
    values = {win: {c: float(rows[win][pid]['sca3_net'])
                    for c, pid in grid.items() if pid in rows[win]}
              for win in WINDOWS}
    print(f"T1_window 台地検査: 全{len(grid)}窓 / "
          + " / ".join(f"{win}済 {len(values[win])}窓" for win in WINDOWS))
    print("第3枠 magic 20261004 / sca3_net は profit由来・円。時刻はサーバー時刻。")
    print("-- = 未完了または数値欠損、対象外 = 提案なし。未完了はゼロに数えません。")
    print("隣接は開始±1・幅固定、および開始固定・幅±1の上下左右（斜めを除く）。")
    for win in WINDOWS:
        for field in ('sca3_net', 'sca3_n'):
            print(f"\n{win} {field} / 開始時刻 × レンジ幅")
        ...
        c = (9, 2)
        if c in v:
            print(f"{win} 参照 S3001（EA既定の中心点） = {v[c]:+,.0f}")
            neighborhood(c, v, grid)
        else:
            print(f"{win} 参照 S3001（EA既定の中心点）: 比較待ち")
