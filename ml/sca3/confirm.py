...
【この掃引に基準行が無い理由】proposals.csv は全案 Sca3Enable=true なので、
第3セッション無しの行が results.csv に存在しない。DD比較には基準のDDが要るので、
ここで測る。

【基準の取り方】第2セッション（S2046）は採用済みとして有効にした状態を基準にする。
測りたいのは「第2セッションに対する増分」であって、第2＋第3の合計ではない。
...
DEPOSIT = 500000
RUN_TIMEOUT = 2400
# 最良年除外後のOOSが大きい順。S3001は前ラウンド由来の中心点で、比較の足場。
CANDIDATES = ["S3018", "S3017", "S3016", "S3001"]
...
MAG = {20260640: "pb", 20261002: "sca1", 20261003: "sca2", 20261004: "sca3"}
...
def run(label, book, params, window):
    ...
    run_id = f"cf3_{window.lower()}_{label}_{stamp}_{uuid.uuid4().hex[:4]}"
    p = dict(book)
    ...
    if dst.exists():
        for r in csv.DictReader(open(dst, encoding="utf-8")):
            ...
            if r["entry"] == "0":
                ...
                st[k]["net"] += float(r["profit"])   # profit_jpy はJPY建てでは使えない
    kill()
    ...
    log(f"RUN_END {label} {window} net={row.get('net')} dd={row.get('dd_pct')} "
        f"sca3={row.get('sca3_net')} {row['elapsed']}s")
    return row
...
def main():
    ...
    jobs = []
    for w in ("IS", "OOS"):
        jobs.append(("BASELINE", GOLD_ONLY, {"Sca3Enable": False}, w))
        for pid in CANDIDATES:
            jobs.append((pid, GOLD_ONLY, json.loads(props[pid]["parameter_json"]), w))
        jobs.append(("FULL_BASE", FULL_BOOK, {"Sca3Enable": False}, w))
