"""SCA GOLD 第4セッションの掃引ドライバ（XM端末・直列・再開可能）。
...
【判定】第4セッション枠（magic 20261005）だけを見ても意味がない。ブック全体で
純益とDDがどう動くかで見る。取引が増えればDDも増えるのが自然なので、
同じ純益倍率を固定ロットで出した場合のDD倍率（倍率^0.71 で内挿）と突き合わせる。
それより小さくなければ、素直にロット倍率を上げるほうが得。

【基準】第2(13-15時)・第3(9-11時)を有効にした状態を基準にする。測りたいのは
既存3枠に対する増分であって、第2+第3+第4の合計ではない。枠が増えるほど既存枠と
同じ値動きを取る余地が増えるので、単独測定より厳しい条件になる。
"""
...
MAGICS = {20260640: "pb", 20261002: "sca1", 20261003: "sca2", 20260641: "pb2", 20261004: "sca3", 20261005: "sca4"}
...
FIELDS = ["proposal_id", "family", "window", "status", "net", "pf", "dd_pct",
          "monthly_pct", "trades",
          "pb_net", "pb_n", "sca1_net", "sca1_n", "sca2_net", "sca2_n", "pb2_net", "pb2_n", "sca3_net", "sca3_n", "sca4_net", "sca4_n",
...
def run_one(name, exe, prop, window):
    ...
    run_id = f"s4_{window.lower()}_{prop['proposal_id']}_{stamp}_{uuid.uuid4().hex[:4]}"
    ...
    for key in ("pb", "sca1", "sca2", "pb2", "sca3", "sca4"):
        if key in st:
    ...
    log(f"RUN_END {prop['proposal_id']} {window} net={row.get('net')} "
        ...
        f"sca3_net={row.get('sca3_net')} sca4_n={row.get('sca4_n')} "
        f"sca4_net={row.get('sca4_net')} {row['elapsed']}s")
    return row
