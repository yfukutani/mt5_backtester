...
【なぜ単独指標の最大値を採らないか】複数案を比較するので、どれか1つの指標で1位になる案は
...
def main():
    ...
    with open(ROOT / "selected.csv", "w", encoding="utf-8", newline="") as fh:
        w = csv.DictWriter(fh, fieldnames=["proposal_id", "description", "IS_ex_best", "OOS_ex_best",
                                           "IS_edge", "OOS_edge", "IS_years", "OOS_years"])
        w.writeheader()
        w.writerows(out)
    print(f"\n-> {ROOT / 'selected.csv'}")
