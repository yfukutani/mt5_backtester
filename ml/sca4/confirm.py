"""基準と有力案を同一セッションで測り直して確定させる。

【なぜ必要か】MT5テスターは現在のスワップ率を全履歴に一律適用するため、測定日が違うと
同じ取引でも損益が動く（docs/sca_gold_exit_20260904.md §4 で最大6%の実測）。
掃引は2時間以上かけて回すので、その中でも基準がずれうる。採否を決める数値は
基準と候補を連続して測り、その中だけで比較する。

【この掃引に基準行が無い理由】proposals.csv は全案 Sca4Enable=true なので、
第4セッション無しの行が results.csv に存在しない。DD比較には基準のDDが要るので、
ここで測る。

【基準の取り方】第2（S2046）＋第3（S3018）セッションを有効にした状態を基準にする。
測りたいのは「第2＋第3セッションに対する第4セッションの増分」である。

あわせてフルブック（暗号3枠込み）でも測り、他の枠に影響が出ないかを見る。
"""
...
MAG = {20260640: "pb", 20261002: "sca1", 20261003: "sca2", 20261004: "sca3", 20261005: "sca4"}
...
def run(label, book, params, window):
    ...
    run_id = f"cf4_{window.lower()}_{label}_{stamp}_{uuid.uuid4().hex[:4]}"
    p = dict(book)
    ...
    if dst.exists():
        for r in csv.DictReader(open(dst, encoding="utf-8")):
            ...
            if r["entry"] == "0":
                ...
                st[k]["net"] += float(r["profit"])   # 口座通貨の損益で比較を統一するため。
    kill()
    ...
    for k in ("pb", "sca1", "sca2", "sca3", "sca4"):
    ...
    log(f"RUN_END {label} {window} net={row.get('net')} dd={row.get('dd_pct')} "
        f"sca4={row.get('sca4_net')} {row['elapsed']}s")
    return row


def main():
    # 候補未定で基準だけの測定を始めないため、端末に触る前に戻る。
    if not CANDIDATES:
        print("候補未設定のため確定測定は行いません。CANDIDATESを設定してください。")
        return
    ...
    jobs = []
    for w in ("IS", "OOS"):
        jobs.append(("BASELINE", GOLD_ONLY, {"Sca4Enable": False}, w))
        for pid in CANDIDATES:
            jobs.append((pid, GOLD_ONLY, json.loads(props[pid]["parameter_json"]), w))
        jobs.append(("FULL_BASE", FULL_BOOK, {"Sca4Enable": False}, w))
    ...
    fields = ["label", "window", "status", "net", "pf", "dd_pct", "monthly_pct",
              ...
              "sca3_net", "sca3_n", "sca4_net", "sca4_n", "elapsed", "run_id"]
