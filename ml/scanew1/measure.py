"""SCA新銘柄横展開をFX 9枠・固定配分のIS/OOSで比較する。新枠の純益と取引数を分離し、端末管理・再開処理・EA差し替え検知はrsimem1を継承する。"""
...
def run(prop, window):
    ...
    run_id = f"sn_{window.lower()}_{prop['proposal_id']}_{stamp}_{uuid.uuid4().hex[:4]}"
    p = dict(BASE)
    ...
    log(f"RUN_END x{mult} {window} net={row.get('net')} dd%={row.get('dd_pct')} "
        f"sca_new_net={row.get('sca_new_net')} sca_new_n={row.get('sca_new_n')} "
