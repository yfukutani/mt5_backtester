...
# 片側95%のt係数（自由度 = シード数 − 1）。Codex指摘（V037）により両側1.96から修正。
# 「シード平均を標本として扱うなら、自由度4の片側t係数（約2.132）を使う」
T_ONESIDED_95 = {1: 6.314, 2: 2.920, 3: 2.353, 4: 2.132, 5: 2.015,
                 6: 1.943, 7: 1.895, 8: 1.860, 9: 1.833}


def mean_ci(vals):
    """平均と、**片側95%**の下限・上限を返す（t分布・自由度 n−1）。"""
    a = np.asarray(vals, dtype=float)
    m = float(a.mean())
    if a.size < 2:
        return m, m, m
    se = float(a.std(ddof=1) / math.sqrt(a.size))
    t = T_ONESIDED_95.get(a.size - 1, 1.645)
...
def main():
    ...
    print("=" * 108)
    print("※ ⚠️ これは**実測した継続成績ではない**（Codex指摘・V037）。")
    print("   最終到達確率 = P(期限内到達) + P(期限切れ)×P(その後到達|期限切れ) の")
    print("   最後の項を**1に置いた**値。「そのまま続ければこの確率で到達する」とは読めない。")
    print("   B/Cは残り時間hを使うため、期限後のルールを別途定義しないと")
    print("   『そのまま継続』自体が定まらない。表示の0.0%は丸めであり真のゼロではない。")
