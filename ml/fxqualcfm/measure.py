...
# --- 採用候補（枠が違うので足し算になることは4例で確認済み）----------------
CAND = {"RsiMechMask_UJ": 124, "PbSlopeATR_UJ": 1.40}

# --- 3件目の候補（第8ラウンドの副産物）------------------------------------
# `ml/fxqual8` の RV06（SCA 2枠にレンジ幅下限 0.00595）は、ブック全体では
# OOS −50,526 / IS −2,109 の**大きな棄却**だった。だが枠別に割ると:
#
#   SCA GBPJPY: OOS −54,524 / IS −5,374   ← これが全部悪い
#   SCA USDJPY: OOS **+3,991** / IS **+3,393**   ← **両窓プラス**
#
# **SCA USDJPY だけに掛ける構成は、第8ラウンドに含まれていない**
# （RV06 は2枠・RV07 は GBPJPY のみ）。枠別の効果は加算的であることが4例で
# 確認できているので、**ブック Δ は +3,991 / +3,393 になるはず**である。
#
# ⚠️ **これは「結果を見てから枠を選んだ」形である。** ただし第7ラウンドの Z007 と違い、
# **選ぶ前に予測が存在した**——`oanda_fx_last_axes_20260915.md` は 2026-09-15 の時点で
# 「B1 は SCA USDJPY の OOS 赤字を黒字に変える（+4,975円）」と書いており、
# 同時に「**金額が小さすぎて採否を論じる意味がない**」とも書いている。
# **後者の評価は正しい。**（+3,991円は OOS 55か月で幾何月利 +0.015pt 相当）
# それでも測るのは、**両窓プラスの候補を取りこぼさないため**である。
SCA_UJ_RANGE = {"ScaFilMask": 1, "ScaFilRangeMin": 0.00595}
...
PROPOSALS = [
    ...
    # --- 3件目の候補を単独で測る（第8ラウンドには UJ 単独の構成が無い）--------
    ("F006", "F000", "SCA USDJPY だけレンジ幅下限 0.00595（予測 OOS +3,991 / IS +3,393）",
     t(PROD, **SCA_UJ_RANGE)),
    ("F007", "F001", "採用候補3件（V001 ＋ PB slope 1.40 ＋ SCA UJ レンジ幅下限）",
     t(PROD, **CAND, **SCA_UJ_RANGE)),
]

# 本番現行 → R036 → R037 → 3件目の候補。窓は OOS/IS/FULL。
# 途中で止まっても「本番現行で残るか」から埋まる並びにする。
ORDER = ["F000", "F001", "F002", "F003", "F004", "F005", "F006", "F007"]
