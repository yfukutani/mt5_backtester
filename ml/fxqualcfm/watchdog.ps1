# fxqualcfm のウォッチドッグ（第17報の確認ラウンド）。
#
# 【塞ぎたい穴】
...
# `FXQUALCFM_END`** で判定し、無ければ `chain.ps1` を起こし直す。
# `chain.ps1` も `measure.py` も冪等（済んだ run は results.csv を見て飛ばす）。
#
# ⚠️ 照合は `*fxqualcfm\chain.ps1*` と名指しする。`*fxqualcfm*` だと
#    このウォッチドッグ自身のパスに必ずヒットして永久に「生きている」と誤判定する
#    （第17報で実際に踏んだ）。
# ⚠️ このファイルは UTF-8 BOM付きで保存すること。
$ErrorActionPreference = 'Continue'
$repo  = 'C:\Users\f\source\repos\mt5_backtester'
$mlog  = Join-Path $repo 'ml\fxqualcfm\measure.log'
$clog  = Join-Path $repo 'ml\fxqualcfm\chain.log'
$wlog  = Join-Path $repo 'ml\fxqualcfm\watchdog.log'
$chain = Join-Path $repo 'ml\fxqualcfm\chain.ps1'
...
Say 'WATCHDOG_START fxqualcfm を見張る（5分おき・最大12時間）'
$deadline = (Get-Date).AddHours(12)
$restarts = 0
while ((Get-Date) -lt $deadline) {
  Start-Sleep -Seconds 300

  if ((Test-Path $mlog) -and (Select-String -Path $mlog -Pattern 'FXQUALCFM_END|FXQUALCFM_ABORT' -Quiet)) {
  ...
  $ch = @(Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.CommandLine -like '*fxqualcfm\chain.ps1*' })
  $mp = @(Get-CimInstance Win32_Process -Filter "Name='python.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.CommandLine -like '*fxqualcfm*measure.py*' })
  if ($ch.Count -gt 0 -or $mp.Count -gt 0) { continue }

  # fxqual8 がまだなら、動いていないのは正常
  $q8 = Join-Path $repo 'ml\fxqual8\measure.log'
  if ((Test-Path $q8) -and -not (Select-String -Path $q8 -Pattern 'FXQUAL8_END|FXQUAL8_ABORT' -Quiet)) { continue }
