"""RSIシグナル記憶ラボをIS/OOSで比較する。倍率は1に固定し、寿命・失効・消費の小さな効果をRSI 3枠の純益で評価する。端末管理と再開処理はfxweight1を継承する。"""
...
def run(prop, window):
    ...
    run_id = f"rm_{window.lower()}_{prop['proposal_id']}_{stamp}_{uuid.uuid4().hex[:4]}"
    p = dict(BASE)
