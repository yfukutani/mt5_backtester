...
ROOT = REPO / "ml" / "scaexit2"
PROPOSALS = ROOT / "proposals.csv"
RESULTS = ROOT / "results.csv"
CONFIG_DIR = ROOT / "configs"
RUN_DIR = ROOT / "runs"
DEAL_DIR = ROOT / "run_deals"
PROGRESS = ROOT / "run_hx.log"
LOCK = ROOT / "run_hx.lock"
...
# 基準の SCA GOLD 建玉（集中度・年次一貫性の比較対象）
BASELINE_POS: dict[str, dict] = {"IS": {}, "OOS": {}}
...
FIELDS = [
    ...
    "ea_sha256",
    "pos_common", "pos_base_only", "pos_cand_only", "changed_positions",
    "improved", "worsened", "top1_share", "top3_share", "years_up", "years_down",
...
def sca_positions(deal_path: Path) -> dict[int, tuple[int, float]]:
    """SCA GOLD の建玉ごとの (エントリー年, 損益円)。"""
    ...
    for pid, i in ins.items():
        ...
        if not o:
            continue
        year = _dt.fromtimestamp(int(i["time"]), _tz.utc).year
        out[pid] = (year, sum(float(x["profit_jpy"]) for x in o))
    return out


def robustness(base: dict, cand: dict) -> dict:
    """改善が広く分布しているか、それとも少数に依存しているかを測る。

    【なぜ必要か】scaexit1 で最上位だった案は OOS +93.91% だったが、
    改善の59.7%が1件・悪化建玉の方が多い・保有時間が倍という中身だった。
    採用候補として通してから人手で気づくのでは遅い。判定に組み込む。
    """
    ...
    if not common:
        return {}
    deltas = [(p, cand[p][1] - base[p][1]) for p in common]
    net = sum(d for _, d in deltas)
    changed = [d for d in deltas if abs(d[1] if isinstance(d, tuple) else d) > 0.5]
    changed = [(p, d) for p, d in deltas if abs(d) > 0.5]
    gains = sorted((d for _, d in deltas if d > 0.5), reverse=True)
    losses = [d for _, d in deltas if d < -0.5]
    top1 = (gains[0] / net) if (gains and net > 0) else 0.0
    top3 = (sum(gains[:3]) / net) if (gains and net > 0) else 0.0

    per_year: dict[int, float] = {}
    for p, d in deltas:
        per_year[base[p][0]] = per_year.get(base[p][0], 0.0) + d
    y_pos = sum(1 for v in per_year.values() if v > 0.5)
    y_neg = sum(1 for v in per_year.values() if v < -0.5)

    return {
        "pos_common": len(common),
        "pos_base_only": len(set(base) - set(cand)),
        "pos_cand_only": len(set(cand) - set(base)),
...
def detail(window: str, v: dict, b: dict) -> str:
    ...
    conc = ""
    if v.get("changed_positions") is not None and v.get("top1_share") is not None:
        conc = (f", 変化建玉={v.get('changed_positions')}"
                f"(改善{v.get('improved')}/悪化{v.get('worsened')})"
                f", 上位1件={100*float(v.get('top1_share') or 0):.0f}%"
                f", 年次+{v.get('years_up')}/-{v.get('years_down')}")
    return (f"{window} net={v['net']:.0f} ({100*(v['net']/b['net']-1):+.2f}%), "
            f"PF={v['pf']:.4f} ({100*(v['pf']/b['pf']-1):+.2f}%), "
            f"DD={v['dd_pct']:.4f}% ({v['dd_pct']-b['dd']:+.4f}pt), "
            f"trades={v['trades']} ({100*(v['trades']/b['trades']-1):+.2f}%){sca}{conc}")
...
def classify_is(v: dict) -> tuple[str, str, str]:
    ...
    # 【集中度ゲート】改善が少数の取引に依存している案は通さない。
    # scaexit1 の最上位案は改善の59.7%が1件・悪化建玉の方が多かった。
    # 建玉集合が変わる案（＝エントリーの母集団を変えてしまう案）も同様に外す。
    if v.get("pos_base_only", 0) or v.get("pos_cand_only", 0):
        return "POPULATION_CHANGED", "SCA_POSITION_SET_DIFFERS", reason
    # 集中度はここでは棄却しない。TP到達取引はISで242件中15件しかなく、どの案も
    # 必然的に集中する。ここで落とすとOOSが一切測れず「面か点か」を両窓で判別できない。
    # survivor 系の判定名に注記として残し、採否は最終集計で集中度・年次一貫性まで見て決める。
    concentrated = (float(v.get("top1_share") or 0.0) > 0.60 or
                    int(v.get("worsened") or 0) > int(v.get("improved") or 0))
    # 本ラウンドの目的は収益増強。DDを悪化させずに純益を1%以上伸ばすこと。
    if net_ratio >= 1.01 and dd_delta <= 0.50:
        if concentrated:
            return "IS_SURVIVOR_CONCENTRATED", "PROFIT_UP_BUT_FEW_TRADES", reason
...
def classify_oos(is_reason: str, v: dict) -> tuple[str, str, str]:
    ...
    if v.get("pos_base_only", 0) or v.get("pos_cand_only", 0):
        return "POPULATION_CHANGED", "SCA_POSITION_SET_DIFFERS", reason
    oos_concentrated = (float(v.get("top1_share") or 0.0) > 0.60 or
                        int(v.get("worsened") or 0) > int(v.get("improved") or 0))
    if net_ratio >= 1.0 and dd_delta <= 0.50:
        if oos_concentrated:
            return "ADOPT_CANDIDATE_CONCENTRATED", "BOTH_UP_BUT_FEW_TRADES", reason
...
def run_once(name: str, exe: str, proposal: dict, window: str) -> dict:
    ...
    run_id = f"hx_{window.lower()}_{proposal['proposal_id']}_{stamp}_{uuid.uuid4().hex[:6]}"
    ...
    rb = robustness(BASELINE_POS.get(window, {}), sca_positions(dst))
    row.update(rb)
    if b.get("pb"):
...
def rebase(name: str, exe: str, window: str) -> None:
    ...
    with _baseline_lock:
        ...
        bd = DEAL_DIR / f"{row['run_id']}_deals.csv"
        pos = sca_positions(bd)
        if pos:
            BASELINE_POS[window] = pos
        ...
        try:
            ...
            decision, gate, reason = classify_is(
                 ...
                 **{k: row.get(k, 0) for k in
                    ("pos_base_only", "pos_cand_only", "top1_share",
                     "improved", "worsened", "years_up", "years_down")}})
...
def load_baseline(window: str) -> None:
    ...
    if BASELINE[window] is not None:
        return
    name = f"scaexit2_baseline_{window.lower()}"
    for base in (ROOT / "runs", REPO / "results", Path.cwd() / "results"):
        ...
        if p.exists():
            ...
            for row in csv.reader(open(p, encoding="utf-8")):
                if len(row) >= 2:
                    vals[row[0]] = row[1]
            bd = COMMON / f"scaexit2_baseline_{window.lower()}_deals.csv"
            pb, sca = sleeve_net(bd)
            BASELINE_POS[window] = sca_positions(bd)
            BASELINE[window] = {
