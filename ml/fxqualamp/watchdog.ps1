# fxqualamp のウォッチドッグ（第17報の追加ラウンド）。
#
# 【塞ぎたい穴】
# 並行セッションの ml/fxqual9 と同じ時間帯に走るので、`measure.py` の
...
# `FXQUALAMP_END`** で判定し、無ければ `chain.ps1` を起こし直す。
# `chain.ps1` も `measure.py` も冪等（済んだ run は results.csv を見て飛ばす）。
#
# ⚠️ 照合は `*fxqualamp\chain.ps1*` と名指しする。`*fxqualamp*` だと
#    このウォッチドッグ自身のパスに必ずヒットして永久に「生きている」と誤判定する
#    （第17報で実際に踏んだ）。
# ⚠️ このファイルは UTF-8 BOM付きで保存すること。
$ErrorActionPreference = 'Continue'
$repo  = 'C:\Users\f\source\repos\mt5_backtester'
$mlog  = Join-Path $repo 'ml\fxqualamp\measure.log'
$clog  = Join-Path $repo 'ml\fxqualamp\chain.log'
$wlog  = Join-Path $repo 'ml\fxqualamp\watchdog.log'
$chain = Join-Path $repo 'ml\fxqualamp\chain.ps1'
...
Say 'WATCHDOG_START fxqualamp を見張る（5分おき・最大12時間）'
$deadline = (Get-Date).AddHours(12)
$restarts = 0
while ((Get-Date) -lt $deadline) {
  Start-Sleep -Seconds 300

  if ((Test-Path $mlog) -and (Select-String -Path $mlog -Pattern 'FXQUALAMP_END|FXQUALAMP_ABORT' -Quiet)) {
  ...
  $ch = @(Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.CommandLine -like '*fxqualamp\chain.ps1*' })
  $mp = @(Get-CimInstance Win32_Process -Filter "Name='python.exe'" -ErrorAction SilentlyContinue |
            Where-Object { $_.CommandLine -like '*fxqualamp*measure.py*' })
  if ($ch.Count -gt 0 -or $mp.Count -gt 0) { continue }

  # fxqual8 がまだなら、動いていないのは正常
  $qprev = Join-Path $repo 'ml\fxqual10\measure.log'
  if ((Test-Path $qprev) -and -not (Select-String -Path $qprev -Pattern 'FXQUAL10_END|FXQUAL10_ABORT' -Quiet)) { continue }
