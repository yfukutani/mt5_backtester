"""SCA枠のrisk%化を実効化した状態で測り直す30案の掃引（FULL/OOS）。
fxrisk1 は SCA の発注経路が LotRisk() を通らず、FxRiskMask の bit3/bit4 が
一度も効いていなかった（R008/R009/R016-R021 が mask=0 の R003 と完全一致）。
2026-09-15 の ScaBaseLot() 修正後の初回測定であり、fxrisk1 とは EA が違う。
S001/S002 は修正がSCA以外へ波及していないことの対照で、
fxrisk1 の R003 / R013 と完全一致しなければならない。
再開・EA差し替え検知・スリープ抑止・端末回収はfxrisk1を継承する。
"""
...
def run(prop, window):
    ...
    run_id = f"fs_{window.lower()}_{prop['proposal_id']}_{stamp}_{uuid.uuid4().hex[:4]}"
    p = dict(BASE)
...
def mark_blown(row):
    ...
    if row["proposal_id"] == "S001" and row["status"] == "OK" and trades is not None and trades > 0:
...
def main():
    ...
    try:
        log(f"FXRISK2_START jobs={len(jobs)} done={len(done)}")
        for prop, window in jobs:
            # S001未取得で後続を測ると、破綻を比較判定できないため中断する。
            if prop["proposal_id"] != "S001" and window not in _baseline_trades:
                raise RuntimeError(f"{window}のS001基準取引数がありません")
            row = run(prop, window)
            mark_blown(row)
            append_result(row)
            if prop["proposal_id"] == "S001" and window not in _baseline_trades:
                raise RuntimeError(f"{window}のS001取得失敗。再開時に再試行してください")
    finally:
        ctypes.windll.kernel32.SetThreadExecutionState(0x80000000)
    log(f"FXRISK2_END -> {OUT}")
