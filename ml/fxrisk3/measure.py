"""risk% を最小ロットの壁を越える水準に校正し直した30案の掃引（FULL/OOS）。

fxrisk2 は risk% を 0.05〜0.5% と低く振ったため、ロットが 0.01 に丸め戻されて
固定ロットと同じ結果になった（S003 が S001 と純益・DD・取引数まで完全一致）。
本ラウンドは非複利で 0.5/1.0/2.0%、equity連動で 0.25/0.5/1.0% を使う。
generate_proposals.py は 0.25% 未満を例外で弾く。

T001 / T002 は fxrisk2 の S001 / S002 とパラメータが同一なので結果を転記済み。
再開・EA差し替え検知・スリープ抑止・端末回収は fxrisk2 を継承する。
"""
...
BASE = {
    ...
    # EA既定値の変更で比較対象外のラボが混入しないよう、無効値を固定する。
    "ScaNewEnable": False,
    ...
    # RefCap_*は3軸比較を妨げないようBASEに置かず、各案で必須指定する。
}
...
def run(prop, window):
    ...
    run_id = f"ft_{window.lower()}_{prop['proposal_id']}_{stamp}_{uuid.uuid4().hex[:4]}"
    p = dict(BASE)
...
def mark_blown(row):
    ...
    if row["proposal_id"] == "T001" and row["status"] == "OK" and trades is not None and trades > 0:
...
def main():
    ...
    try:
        log(f"FXRISK3_START jobs={len(jobs)} done={len(done)}")
        for prop, window in jobs:
            # T001未取得で後続を測ると、破綻を比較判定できないため中断する。
            if prop["proposal_id"] != "T001" and window not in _baseline_trades:
                raise RuntimeError(f"{window}のT001基準取引数がありません")
            row = run(prop, window)
            mark_blown(row)
            append_result(row)
            if prop["proposal_id"] == "T001" and window not in _baseline_trades:
                raise RuntimeError(f"{window}のT001取得失敗。再開時に再試行してください")
    finally:
        ctypes.windll.kernel32.SetThreadExecutionState(0x80000000)
    log(f"FXRISK3_END -> {OUT}")
