...
def r_ratio(ml_min: float, cap: float) -> float | None:
    ...
    🔴 **R は外挿の道具ではない。** 床は「cap を守って建てた**瞬間**に保証される維持率」で、
    **谷の瞬間の使用証拠金は cap とは無関係に決まっている。**
    実例: `M006 OOS` の谷は equity 1,180,534 / 使用証拠金 1,236,016
    ＝ **使用証拠金が equity の 104.7%** で、cap90 をとっくに超えている。
    つまり R が測っているのは「cap からの乖離」ではなく
    **「建てた後にどれだけ equity が落ちたか」**である。
    cap を絞れば建玉が小さくなり equity の落ち方も小さくなるはずなので、
    **R は cap を絞るほど改善する方向に動く**と予想される（未確認）。
    """
    fl = floor_pct(cap)
