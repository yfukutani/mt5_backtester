...
【🔴 事前登録: cap と最小維持率の関係（2026-09-20・走る前に書く）】
`MarginCapPct=c` は**発注の瞬間**に `使用証拠金 ≦ equity × c/100` を掛ける。
したがって**発注直後の維持率には床がある**:

    床 = equity / (equity × c/100) × 100 = **10000 / c** [%]
    cap33 → 303% / cap50 → 200% / cap70 → 143% / cap90 → 111%

**建てた後は equity だけが落ちるので、実際の最小維持率は床より下**になる。
そこで **R ≡ 最小維持率 ÷ 床** を定義する。

> ⚠️ **先に「何に対して不変でないか」を書く。** R は
> **倍率・枠構成・窓・端末（フィード）には不変でない。**
> 不変性を主張するのは **「同じブック・同じ窓の中で cap だけを振ったとき」**に限る。
> **それが本当かどうかが、本ラウンドで初めて測れる**（cap 33/50/70 が同一ブック・同一窓にある）。

並行セッションの実測（XM・全複利・倍率3・cap90・OOS）:

| 構成 | 最小維持率 | 床 | **R** |
|---|---:|---:|---:|
| Carry **あり**（`M003`） | 54.17%（**初日**） | 111.1% | **0.488** |
| Carry **抜き**（`M006`） | 95.51%（窓の中ほど） | 111.1% | **0.860** |

**Carry を外すと谷が初日から窓の中ほどへ動いた**＝水準の改善ではなく**原因の除去**。
ただし **95.51% でも 100% は割っている。**

**この R をそのまま当てはめた予想（外れてよい。外れたら R は cap 不変でないと分かる）:**

| 本ラウンドの run | cap | 床 | **予想する最小維持率** | 生存 |
|---|---:|---:|---:|---|
| `O004`（S033・Carry無） | 33 | 303% | **約 260%** | ○ |
| `O000`/`O001`/`O020`…（S050・Carry無） | 50 | 200% | **約 172%** | ○ |
| `O002`/`O003`（S070・Carry無） | 70 | 143% | **約 123%** | ○（ただし薄い） |
| **`O011`（全複利 倍率2・cap90・Carry有）** | 90 | 111% | **約 54%** | **×（切られる）** |
| **`O010c`/`O011c`（全複利 cap50・Carry有）** | 50 | 200% | **約 98%** | **際どい。ここが本番** |
| **`O012`（全複利 倍率3・Carry抜き・cap70）** | 70 | 143% | **約 123〜128%** | **○と予想** |
| **`O013`（同・cap50）** | 50 | 200% | 約 172〜178% | ○ |

🔴 **`O010c`/`O011c` が最も情報量が多い。** 予想 98% は 100% のすぐ下＝**どちらにも転ぶ。**
**「Carry を抱えたまま cap を絞れば OANDA で生き残れるのか」**という、
本プロジェクトで一度も答えていない問いの答えがここで出る。

> [!warning] ⚠️ **上の予想は「R が cap 不変」を仮定しており、その仮定は疑わしい**
> （並行セッションの指摘・2026-09-20）。**床は「cap を守って建てた瞬間に保証される維持率」であって、
> 谷の瞬間の使用証拠金は cap とは無関係に決まっている。**
> 実データ: `M006 OOS` の谷は **equity 1,180,534 / 使用証拠金 1,236,016**
> ＝**使用証拠金は equity の 104.7% で、cap90 をとっくに超えている**
> （建てた時点では 90% 以下だったが、その後 equity が落ちて比率が上がった）。
>
> **つまり R は「cap からの乖離」ではなく「建てた後にどれだけ equity が落ちたか」を測っている。**
> そして **cap を絞れば建玉が小さくなり、その後の equity 低下も小さくなるはずなので、
> R は cap を絞るほど改善する方向に動く**と考えられる。
>
> **したがって予想 98% は下振れ側の見積もり（下界）である。**
> **実測が 98% より上に出たら、それは「R が cap 不変でなかった」＝上の理屈が正しい、と読む。**
> **98% より下に出たら、cap を絞っても足りないということになる。**
> どちらでも情報になるが、**R を外挿の道具として使ってはいけない**ことは変わらない。
> 本ラウンドは cap 33/50/70 が同一ブック・同一窓にあるので、
> **R が cap とともに単調に改善するかを初めて直接見られる。**
...
def full_comp(mult, cap, carry=True):
    ...
    p["En_CARRY"] = carry         # T036系(E04)では False。ここが2系統の枠構成の差
...
PROPOSALS = [
    ...
    # --- ★ 目標にいちばん近い構成を、100% ロスカットが本当に効く端末で ---------
    # 並行セッションの実測（XM・倍率3・Carry 抜き・cap90）:
    #   OOS 3.614% / 最小維持率 95.51%    IS 6.407% / 最小維持率 **99.09%**
    #
    # 🔴 **IS の 6.407% を「目標達成」として扱ってはいけない。**
    #    **判定窓は OOS（3.614%）である**（本タスクの目標は「OOS 窓・複利の幾何平均で月利6%」）。
    #    IS はパラメータを合わせた窓なので、そこで 6% を超えるのは達成ではない。
    #    **目標に対する現在地は OOS 3.614% であって、6% には遠い。**
    #
    # ここで測りたいのは **月利ではなく「維持率が 100% を超えるか」**である。
    # 維持率 99.09% は 100% にほぼ接していて、割ったティックは 662 本（全体の 0.0007%）＝
    # **あと一息で生存する。cap を少し絞れば超える見込みが高い。**
    # ⚠️ **OANDA フィードなので月利は XM の数字より落ちる**（第14報のフィード差 −27%）。
    #    **月利の再現は目的ではない。**
    ("O012", "fxqual17/M006",
     "★Carry 抜き・倍率3・**cap70**。XM の cap90 は IS 99.09% で届かなかった。絞れば超えるか",
     full_comp(3, 70, carry=False)),
    ("O013", "O012", "★同・**cap50**。cap70 でも足りないときの退避先",
     full_comp(3, 50, carry=False)),
]
...
JOBS = [
    ...
    ("O020", "IS"),    # 4.    同・UJ
    ("O022", "IS"),    # 5.    合成
    # 6-9. ★ 目標にいちばん近い構成（XM で IS 6.407%・維持率 99.09%）を、
    #      100% ロスカットが本当に効く端末で。**枠の質の OOS より先に取る**——
    #      「6% を超えていて、かつ生き残る構成があるか」のほうが決定的だから。
    ("O012", "IS"), ("O012", "OOS"),
    ("O013", "IS"), ("O013", "OOS"),
    ("O023", "OOS"),   # 10.
    ("O020", "OOS"),   # 11.
