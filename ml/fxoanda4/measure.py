...
PROPOSALS = [
    ...
    # --- ★★ いま最有力の構成に「余裕」を作れるか -----------------------------
    # 並行セッションの `M002 OOS`（XM・倍率2・Carry 有・cap90）が
    # **本プロジェクトで初めて維持率 100% を超えた**: 101.47%・100%割れ 0 ティック・R=0.913。
    # しかも **倍率3・Carry 抜き（3.614%）より月利が高い（3.849%・+0.235pt）。**
    # → **「Carry を外す」より「倍率を1段下げる」ほうが安い**（−0.99pt で 95.5% 止まり vs
    #    −0.758pt で 101.47%）。**Carry を外す道は優先度が下がった。**
    # ⚠️ ただし **101.47% はロスカットまで 1.47pt しかない。**
    #    スプレッド拡大・証拠金率変更・約定の滑りを考えると実運用では余裕が無い。
    # → **cap で「生存」から「余裕」へ動かせるか**を測る。これが cap を絞る道の本命になった。
    ("O014", "fxqual17/M002", "★★倍率2・Carry 有・**cap70**。101.47% を実運用に耐える余裕まで上げられるか",
     full_comp(2, 70)),
]
...
JOBS = [
    ...
    # 7-14. ★★ **cap で救出できるか** — 落ちている窓から先に測る。
    #
    # 🔴 **2026-09-20 の再整理**（`fxqual17` 全16run 完了・並行セッションの訂正を反映）。
    #    **両窓で生存するかで見ると、話が入れ替わった:**
    #
    #      倍率2・Carry 有  : OOS 101.47% ✅ / **IS 96.51% ❌** → 失格
    #      倍率2・Carry 抜き: OOS 105.13% ✅ / IS 102.25% ✅ → **両窓生存の最良（OOS 3.186%）**
    #      倍率3・Carry 抜き: OOS  95.51% ❌ / IS  99.09% ❌ → 失格（ただし IS はあと 0.91pt）
    #
    #    **「倍率2・Carry 有が初の生存構成」は OOS だけを見た誤りだった**（前便で撤回済み）。
    #
    # **救出できたときの取り分が大きい順に測る:**
    #   `O014`(倍率2・Carry有) が救えれば **IS 7.033% / OOS 3.849%**（両窓生存の最良より +2.5pt/+0.66pt）
    #   `O012`(倍率3・Carry抜き) が救えれば **IS 6.407% / OOS 3.614%**（同 +1.9pt/+0.43pt）
    # **どちらも落ちているのは主に IS なので、IS から測る。**
    ("O014", "IS"),    # 7.  ★★ cap70 は 倍率2・Carry有 の IS 96.51% を救えるか
    ("O012", "IS"),    # 8.  ★★ cap70 は 倍率3・Carry抜き の IS 99.09% を救えるか（あと 0.91pt）
    ("O014", "OOS"),   # 9.
    ("O012", "OOS"),   # 10. こちらは 95.51% で、IS より遠い
    ("O011c", "IS"),   # 11. cap50 まで絞る（cap70 で足りなかったときの退避先）
    ("O013", "IS"),    # 12.
    ("O011c", "OOS"), ("O013", "OOS"),   # 13-14.
    ("O023", "OOS"),   # 14.
    ("O020", "OOS"),   # 15.
    ("O022", "OOS"),   # 16.
    ("O002", "IS"),    # 17. cap70 の IS
    ("O003", "OOS"),   # 18. cap70 の通期OOS（fxoanda3 で起動失敗した穴）
    ("O010c", "IS"), ("O010c", "OOS"),   # 19-20. 倍率1・cap50
    ("O011", "OOS"),   # 21. cap90 は OANDA で建てられるのか（落ちると予想）
