"""PB/SCA GOLD の risk% サイジングを XM端末で掃引する。

OANDA 5端末はすべて MT5 の LiveUpdate（6140→6180）に捕まり使用不能なため、
既に 6180 へ更新済みでクリーンな XM端末で測る。1台のみなので直列実行。

チャート銘柄は USDJPY にする。GOLD にすると JPY建て口座の通貨換算に XAUJPY が要り、
XMの XAUJPY 履歴が 2025.09.17 以降しかないため OOS 期間が no history data で落ちる
（GOLD 自体は 2008.02.19 から揃っている）。ml/deploy50b の XM 測定も USDJPY で成功している。
...
# 【2026-09-05】OANDA 5端末はすべて LiveUpdate に捕まり使用不能。XM端末で測る。
TERMINALS = [
...
def run_one(name, exe, label, mask, risk, refcap, boost, window):
    ...
    run_id = f"xmgsz_{window.lower()}_{label}_{stamp}_{uuid.uuid4().hex[:4]}"
