# fxqual14 のウォッチドッグ（第26報）。
#
# 【塞ぎたい穴】`measure.py` の `acquire_lock()` が負けて**黙って return する**と、
# `chain.ps1` は「CHAIN_END exit=0」を出して正常終了する。
# **1run も走っていないのに成功に見える。** だから判定は exit code ではなく
# `measure.log` の `FXQUAL14_END` で行う。
#
# ⚠️ 照合は `*fxqual14\chain.ps1*` と名指しする。`*fxqual14*` だと
...
$wlog  = Join-Path $repo 'ml\fxqual14\watchdog.log'
$chain = Join-Path $repo 'ml\fxqual14\chain.ps1'
...
Say 'WATCHDOG_START fxqual14 を見張る（5分おき・最大14時間）'
$deadline = (Get-Date).AddHours(14)
$restarts = 0
while ((Get-Date) -lt $deadline) {
  Start-Sleep -Seconds 300

  if ((Test-Path $mlog) -and (Select-String -Path $mlog -Pattern 'FXQUAL14_END|FXQUAL14_ABORT' -Quiet)) {
