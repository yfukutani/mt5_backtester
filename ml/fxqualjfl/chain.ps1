# fxqualjfl（第21報・分母フロア）を fxqualfloor の後ろに繋ぐ。
#
# ⚠️ **このラウンドは EA を変更している**（LotFloorMask / LotFloorRatio を追加）。
#    したがって自分でデプロイして再コンパイルする。**fxqualfloor が完全に終わる前に
#    コンパイルすると、走行中の run が2つのバイナリに割れる。**
#    FXQUALFLOOR_END に加えて、ロックと metatester64 の不在も条件にする。
#
# ⚠️ ここを飛ばすと静かに嘘の実測が残る。MT5 は EA が持っていない input を
#    黙って無視するので、6案すべてが対照と完全同値になり、
#    それが「効かなかった」という結論に化ける（第16報で実際に踏んだ形）。
#
# ⚠️ このファイルは UTF-8 BOM付きで保存すること。BOM無しだと PowerShell 5.1 が
#    ANSI として読み、日本語コメントが化けて構文エラーで即死する。
$ErrorActionPreference = 'Continue'
$repo    = 'C:\Users\f\source\repos\mt5_backtester'
$py      = 'C:\Users\f\AppData\Local\Programs\Python\Python314\python.exe'
$experts = 'C:\Users\f\AppData\Roaming\MetaQuotes\Terminal\BAC624F09E3C5D5AFDD21CE91C0B879D\MQL5\Experts'
$med     = 'C:\Users\f\AppData\Roaming\XMTrading MT5\MetaEditor64.exe'
$lock    = Join-Path $repo 'ml\fxmargin3\measure.lock'
$log     = Join-Path $repo 'ml\fxqualjfl\chain.log'

function Say([string]$m) {
  $line = "{0} {1}" -f (Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ'), $m
  Add-Content -Path $log -Value $line -Encoding utf8
}

function LockBusy {
  if (-not (Test-Path $lock)) { return $false }
  $t = (Get-Content $lock -Raw -ErrorAction SilentlyContinue)
  if ($null -eq $t) { return $false }
  $t = $t.Trim()
  if ($t -notmatch '^\d+$') { return $false }
  return $null -ne (Get-Process -Id ([int]$t) -ErrorAction SilentlyContinue)
}

function Quiet {
  if (Get-Process metatester64 -ErrorAction SilentlyContinue) { return $false }
  if (LockBusy) { return $false }
  return $true
}

Say 'CHAIN_START(fxqualjfl) fxqualfloor の完了を待つ（最大12時間）'
$prev = Join-Path $repo 'ml\fxqualfloor\measure.log'
$deadline = (Get-Date).AddHours(12)
while ((Get-Date) -lt $deadline) {
  if ((Test-Path $prev) -and (Select-String -Path $prev -Pattern 'FXQUALFLOOR_END|FXQUALFLOOR_ABORT' -Quiet)) {
    if (Quiet) { break }
  }
  Start-Sleep -Seconds 60
}
if ((Get-Date) -ge $deadline) { Say 'CHAIN_ABORT 12時間待っても fxqualfloor が終わらなかった'; exit 1 }

Start-Sleep -Seconds 30
if (-not (Quiet)) { Say 'CHAIN_ABORT 待機後にロック／テスターが動いていた。EAは触らない'; exit 1 }

$deployed   = Join-Path $experts 'MIX_EA_SIMVERIFY.mq5'
$deployedEx = Join-Path $experts 'MIX_EA_SIMVERIFY.ex5'
$srcMq5     = Join-Path $repo 'experts\MIX_EA_SIMVERIFY.mq5'

if (-not (Select-String -Path $srcMq5 -Pattern 'LotFloorMask' -Quiet)) {
  Say 'CHAIN_ABORT リポジトリの .mq5 に LotFloorMask が無い。パッチが当たっていない'
  exit 1
}
if (-not (Select-String -Path $srcMq5 -Pattern 'LotFloorDist' -Quiet)) {
  Say 'CHAIN_ABORT リポジトリの .mq5 に LotFloorDist が無い。入力だけで実装が無い'
  exit 1
}

# デプロイとコンパイルの間だけ、測定ロックを自分の PID で押さえる。
# 並行セッションの chain がこの隙に measure.py を起動すると、
# **走行中のテスターの下で .ex5 が差し替わる。**
Set-Content -Path $lock -Value $PID -Encoding ascii
Say ("LOCK_HELD pid={0} デプロイとコンパイルの間だけ押さえる" -f $PID)

Copy-Item $srcMq5 $deployed -Force
$clog = Join-Path $experts 'c_simverify_lotfloor.log'
& $med "/compile:$deployed" "/log:$clog"
Start-Sleep -Seconds 3
$text = ''
if (Test-Path $clog) { $text = [System.Text.Encoding]::Unicode.GetString([System.IO.File]::ReadAllBytes($clog)) }
$res = ($text -split "`r?`n" | Where-Object { $_ -match '^Result:' } | Select-Object -Last 1)
Say ("COMPILE {0}" -f $res)
if ($res -notmatch '0 errors') {
  Remove-Item $lock -Force -ErrorAction SilentlyContinue
  Say 'CHAIN_ABORT コンパイルに失敗した。走らせない（ロックは返した）'
  exit 1
}

# コンパイル後の3点確認（fxqual8/fxqual9 と同じ作法）。
$hasA = Select-String -Path $deployed -Pattern 'LotFloorMask' -Quiet
$hasB = Select-String -Path $deployed -Pattern 'LotFloorDist' -Quiet
$same = ((Get-FileHash $deployed).Hash -eq (Get-FileHash $srcMq5).Hash)
$fresh = ((Get-Item $deployedEx).LastWriteTime -ge (Get-Item $deployed).LastWriteTime)
Say ("DEPLOY_CHECK hasMask={0} hasDist={1} sameSrc={2} freshEx={3}" -f $hasA, $hasB, $same, $fresh)
if (-not ($hasA -and $hasB -and $same -and $fresh)) {
  Remove-Item $lock -Force -ErrorAction SilentlyContinue
  Say 'CHAIN_ABORT デプロイ検査に落ちた（ロックは返した）'
  exit 1
}

Remove-Item $lock -Force -ErrorAction SilentlyContinue
Say 'LOCK_RELEASED measure.py に渡す'

Say 'FXQUALJFL を開始する（分母フロア 6案＋対照 × 2窓 = 14run）'
Set-Location $repo
& $py 'ml\fxqualjfl\measure.py' 2>&1 | Add-Content -Path $log -Encoding utf8
$code = $LASTEXITCODE
Say ("CHAIN_END exit={0}" -f $code)

# 落ちていたら1度だけ起こし直す（measure.py は done を読むので途中から再開する）。
$mlog = Join-Path $repo 'ml\fxqualjfl\measure.log'
$ok = (Test-Path $mlog) -and (Select-String -Path $mlog -Pattern 'FXQUALJFL_END' -Quiet)
if (-not $ok) {
  Say 'FXQUALJFL_END が無い。60秒待って1度だけ再開する'
  Start-Sleep -Seconds 60
  & $py 'ml\fxqualjfl\measure.py' 2>&1 | Add-Content -Path $log -Encoding utf8
  Say ("CHAIN_END2 exit={0}" -f $LASTEXITCODE)
}
